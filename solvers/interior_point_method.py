""" Interior Point Solver (IPS) for both linear and non-linear programing solvers.
...
def ips(f_fcn, x0=None, Aeq=None, beq=None, A=None, b=None, xmin=None, xmax=None,
    ...
    neq = Aeq.shape[0] if Aeq is not None else 0  # number of equal linear constraints
    nineq = A.shape[0] if A is not None else 0  # number of inequal linear constraints
    ...
    AA = vstack([-eyex, eyex],"csr") if A is None else vstack([-eyex, eyex, A], "csr")
    bb = r_[-xmin, xmax, b]
    ib = find(bb <= 1e10)  # Bounded constrain, does not need to take into consideration
    if len(ib):
        idxs = [(1, ib)]
    ...
    lmbda = {'lam': lam_lin, 'mu': mu_lin[nx + nx:nx + nx + nineq], 'lower': mu_lin[:nx],
...
if __name__ == "__main__":
    ...
    solution = ips(f2, x0, gh_fcn=gh2, hess_fcn=hess2)

    print(round(solution["f"], 11) + 7.07106725919)
